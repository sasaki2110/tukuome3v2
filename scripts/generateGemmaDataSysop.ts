...
async function generateGemmaTrainingData() {
  ...
  ${labelString}</s>`;
    ...
    console.log(`  学習データ追加済み: ${trainingData.length}件`);
  }

  const outputPath = path.join(process.cwd(), 'public', 'me2gemini', 'learn_sysop.json');
  ...
  // より確実なファイル書き込み方法に変更
  const content = trainingData.map(data => JSON.stringify(data)).join('\n') + '\n';
